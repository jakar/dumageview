...
namespace dumageview
{
  namespace
  {
    ...
    void runNewFileDialog(
      DialogRunner::Handler<R> const& resultHandler,
      QWidget* parent,
      ...
      S&& selector)
    {
      ...
      runner->safeExec(
        boost::hana::compose(resultHandler, execHandler));
    }
  }
    ...
    QFileDialog::Options options)
  {
    runNewFileDialog(
      ...
      [](auto* d) { return d->selectedFiles().value(0); }
    );
  }

  void FileDialogRunner::getOpenFileName(
    Handler<QString> const& resultHandler,
    QWidget* parent,
    QString const& caption,
    QString const& dir,
    ...
    QFileDialog::Options options)
  {
    runNewFileDialog(
      ...
      [](auto* d) { return d->selectedFiles(); }
    );
  }

  void FileDialogRunner::getSaveFileName(
    Handler<QString> const& resultHandler,
    QWidget* parent,
    QString const& caption,
    QString const& dir,
                                                ...
                                                ExtensionSet exts)
  {
    ...
    return conv::qstr(fmt::format(
        "{} ({})", std::move(title), boost::join(std::move(exts), " ")));
  }
}
