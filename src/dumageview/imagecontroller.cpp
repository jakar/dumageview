...
namespace dumageview::imagecontroller {
  ...
  auto ImageController::tryRead(QImageReader& reader, ImageInfo const& info)
  ...
  void ImageController::openImage(QString const& path) {
    std::visit(hana::overload(
                 ...
                 [&](QString const& error) {
                   auto msg =
                     "Could not open image: %1: %2"_qstr.arg(path, error);
                   openFailed(msg);
                 }),
               tryOpen(path));
  }
  ...
  void ImageController::saveImage(QString const& path) {
    ...
    if (!writeOK) {
      saveFailed(
        "Could not save image: %1: %2"_qstr.arg(path, writer.errorString()));
      return;
    }
