...
namespace dumageview::math
{
  ...
  {
    template<class V> struct Scalar;
    template<> struct Scalar<glm::ivec2> { using type = int; };
    template<> struct Scalar<glm::dvec2> { using type = double; };
    template<> struct Scalar<glm::ivec2 const> { using type = int const; };
    template<> struct Scalar<glm::dvec2 const> { using type = double const; };
    template<class V> using ScalarT = typename Scalar<V>::type;
    ...
    template<class V>
    inline constexpr GetterFunc<V> vGetX = [](V& v) -> auto& { return v.x; };

    template<class V>
    inline constexpr GetterFunc<V> vGetY = [](V& v) -> auto& { return v.y; };

    inline constexpr Getter getX{
      {vGetX<glm::ivec2>, vGetX<glm::ivec2 const>},
      {vGetX<glm::dvec2>, vGetX<glm::dvec2 const>}
    };

    inline constexpr Getter getY{
      {vGetY<glm::ivec2>, vGetY<glm::ivec2 const>},
